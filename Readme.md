## Ситчихин Евгений - Software Engineer 

В этом файле представляю небольшую компиляцию информации обо мне. 

Так получилось, что практически все проекты, над которыми я работал - это закрытые управленческие системы 
с приватным доступом к исходным кодам. Тем не менне, в открытом доступе опубликован бэкенд NodeJS сервис, созданный мной в конце 2020 года и небольшой тестовый компонент на Vue.js 

Тестовый компонент на Vue.js. 

Вот ссылка на Vue-компонент (код написан на Javascript, хотя я предпочитаю писать на Typescript (class-style components), и 
пользуюсь препроцессорами Pug для html и Scss для css).
...
**2021 год** Techlead Frontend команды разработки Интернет-магазина Siberian Wellness. Разрабатываем новую версию фронтенд-приложения. Новый дизайн + новые технологии. Интернет-магазин будет состоять из нескольких независимых фронтенд-приложений, реализующих различные части пользовательских сценариев. 

Стек тихнологий: 
Nuxt.js, Vue.js, JavaScript, TypeScript, Pug, Scss

**2017-2020 годы**. Руководил небольшой командой и создавал Web-системы для помощи в управлении объектами коммерческой недвижимости. Ядром является CRM, которая включает в себя несколько web-приложений SPA и REST API бэкенд. Ключевые технологические детали: 
интерактивная SVG-карта карта собираемая на Фронтенде из данных в JSON-формате (выдаются по API). 
Приложение, позволяющее редактировать элементы карты из интерфейса;
Предусмотрена возможность применять различные шаблоны дизайна для сайта, а также возможность дополнительной настройки шаблонов.
Вторая версия продукты - это фронтенд на Nuxt.js, бэкенд ряд сервисов на Node.js. Она была разработана на 80%, но запуск не состоялся, проект был заморожен вместе с началом пандемии.
...
Стек технологий: 
PHP, Laravel, Node.js, JavaScript, TypeScript, Vue.js (vuex, vue-router), Nuxt.js, MySQL, Pug, Scss.
